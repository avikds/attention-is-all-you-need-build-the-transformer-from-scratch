...
def apply_adam_step_to_all_parameters(
    ...
    # Advance the global Adam step counter.
    optimizer_state["t"] += 1
    ...
    for i, param in enumerate(parameter_list):
        ...
        m_t = update_adam_first_moment(
            optimizer_state["m"][i],
            grad,
            beta1,
        )
        v_t = update_adam_second_moment(
            optimizer_state["v"][i],
            grad,
            beta2,
        )
        ...
        # Compute and apply the Adam parameter update.
...
def run_training_loop_for_steps(
    ...
    # Cycle through the available batches as many times as needed.
    ...
    for step in range(1, num_steps + 1):
        src_batch, tgt_batch = batches[(step - 1) % num_batches]
...
def pick_next_token_by_argmax(final_step_logits):
    ...
    """
    return torch.argmax(final_step_logits, dim=-1).to(torch.int64)
...
def select_top_k_candidates(candidate_scores, k):
    # Flatten the beam and vocabulary dimensions.
    flat_scores = candidate_scores.reshape(-1)

    # Select the k highest-scoring candidates in descending order.
    top_scores, top_indices = torch.topk(flat_scores, k=k, dim=0)

    # Recover the original beam index and token ID from each flattened index.
    vocab_size = candidate_scores.size(1)
    beam_indices = top_indices // vocab_size
    token_ids = top_indices % vocab_size

    return {
        "beam_indices": beam_indices.to(torch.long),
...
def mark_finished_beams(token_ids, finished_flags, end_token_id):
    # A beam is finished if it was already finished or if it just
